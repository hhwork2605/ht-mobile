...
`IPricingService.GetEffectivePriceAsync(variantId)` = giá `ProductVariant.BasePrice`/`CompareAtPrice` + áp
`Promotion` đang hiệu lực (đọc `conditions_json`) + đính kèm `PaymentPromotion`. Kết quả cache Redis,
invalidate khi cập nhật giá/KM. Đây là phần được unit-test kỹ nhất (`tests/HtMobile.Application.UnitTests`).

> **Lưu ý:** giá **không phân theo vùng** (đã bỏ `Region`/`PriceByRegion` — xem [ADR-0002](decisions/0002-bo-gia-theo-vung.md)). Giá nằm trực tiếp trên variant.
